...
function deletePatient() {
    ...
    // Supporta sia DELETE che POST
    if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
        parse_str(file_get_contents('php://input'), $_DELETE);
        $patientId = $_DELETE['id'] ?? $_GET['id'] ?? null;
    } else {
        $data = json_decode(file_get_contents('php://input'), true);
        $patientId = $data['id'] ?? $_POST['id'] ?? null;
    }
    ...
    try {
        ...
        // Disattiva il paziente invece di eliminarlo (soft delete)
        // Questo preserva i dati per eventuali audit
        $stmt = $db->prepare("
            UPDATE Users 
            SET is_active = 0,
        ...
        // Log attività
        logActivity($db, $nutritionistId, 'patient_deleted', "Paziente ID $patientId rimosso");
        ...
        sendJSON(['success' => false, 'error' => 'Errore nell\'eliminazione del paziente'], 500);
    }
}
?>